# Welcome to GIRAF

Welcome to the GIRAF project! This section will help you get set up in your first week.

## About GIRAF

GIRAF is an umbrella term for digital tools developed for autistic children with little or no verbal communication. The applications have been developed by students at Aalborg University since 2011, in close cooperation with:

- [Børnehaven Birken](http://bhbirken.dk/) (Kindergarten)
- [Egebakken](https://egebakken.aula.dk/) (School)
- [Center for Autism and ADHD](https://handicaptilbuddene.aalborg.dk/fagcentre/fagcenter-for-autisme-og-adhd) at Aalborg Municipality

## First Week Checklist

1. **Get access to accounts** — See [Accounts & Access](accounts_and_access.md)
2. **Join communication channels** — See [Tools](tools.md)
3. **Understand the ecosystem** — See [Architecture Overview](../Development/architecture.md)
4. **Learn how we work** — See [Process](../Process/index.md)

## Need Help

- **Discord** — Day-to-day questions ([Invite Link](https://discord.gg/dNCkbsezKb))
- **Your supervisor** — Project-level guidance
- **ASE lecturer** — Agile/Scrum process questions
